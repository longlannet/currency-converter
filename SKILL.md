...
description: Frankfurter API / 欧洲央行 ECB 汇率数据查询与货币换算。Use when the task involves currency conversion, exchange rates, FX lookup, or converting amounts between currencies such as USD, EUR, CNY, JPY, GBP, HKD, and others.
metadata:
...
Use this skill for read-only currency conversion and exchange-rate lookup using Frankfurter API data sourced from the European Central Bank (ECB).
...
4. Mention that weekends and ECB holidays may return the latest prior business-day rate when date precision matters.
